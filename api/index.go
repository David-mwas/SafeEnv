...
func initApp() {
	once.Do(func() {
		...
		encKey = []byte(os.Getenv("SAFEENV_SECRET_KEY"))

		if len(encKey) != 32 {
			log.Fatal("Encryption key must be 32 bytes")
		}

		// Mongo connect with timeout
		...
		// Request timeout middleware (VERY IMPORTANT)
...
func db(ctx context.Context) *mongo.Collection {
	return collection
}
...
func encrypt(text string) (string, error) {
	...
	return base64.StdEncoding.EncodeToString(append(iv, data...)), nil
}
...
func RegisterUser(c *gin.Context) {
	...
	_, err := db(ctx).Database().Collection("users").InsertOne(ctx, bson.M{
...
func LoginUser(c *gin.Context) {
	...
	err := db(ctx).Database().Collection("users").
		FindOne(ctx, bson.M{"email": body.Email}).
		Decode(&user)
...
func StoreVariable(c *gin.Context) {
	...
	enc, _ := encrypt(body.Value)

	_, err := db(c.Request.Context()).InsertOne(c.Request.Context(), bson.M{
		"userID": userID,
		"key":    body.Key,
		"value":  enc,
	})
...
func GetUserKeys(c *gin.Context) {
	...
	cur, err := db(c.Request.Context()).Find(c.Request.Context(), bson.M{"userID": userID})
	if err != nil {
	...
	var out []bson.M
	_ = cur.All(c.Request.Context(), &out)
...
func RetrieveVariable(c *gin.Context) {
	...
	err := db(c.Request.Context()).FindOne(c.Request.Context(), bson.M{"key": key}).Decode(&res)
	if err != nil {
		c.JSON(404, gin.H{"error": "not found"})
		return
	}

	val, _ := decrypt(res.Value)

	c.JSON(200, gin.H{"key": key, "value": val})
}

/* =========================
   EMAIL (NON-BLOCKING FIX)
========================= */
...
func sendResetEmail(to, token string) {
	go func() {
		...
		msg := []byte("Subject: Reset\n\nToken: " + token)

		_ = smtp.SendMail(addr, auth, os.Getenv("SMTP_EMAIL"), []string{to}, msg)
	}()
}
